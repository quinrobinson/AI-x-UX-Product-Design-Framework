  ...
  return (
    <button onClick={onClick} onMouseEnter={() => setHovered(true)} onMouseLeave={() => setHovered(false)}
      style={{
        ...
        border: selected ? `1.5px solid ${accent}` : `1px solid ${T.surfaceBorder}`,
        background: selected ? `${accent}12` : T.text,
...
function Field({ label, hint, value, onChange, multiline, placeholder, required }) {
  const base = {
    width: "100%", boxSizing: "border-box", border: `1px solid ${T.surfaceBorder}`,
    borderRadius: 10, padding: "11px 14px", fontSize: 13, fontFamily: "'DM Sans', sans-serif",
    color: T.text, background: T.text, outline: "none", lineHeight: 1.6, resize: "vertical",
    ...
    <div style={{ marginBottom: 20 }}>
      ...
      {multiline
        ? <textarea rows={3} value={value} onChange={e => onChange(e.target.value)} placeholder={placeholder} style={base}
            onFocus={e => e.target.style.borderColor = "#14B8A6"} onBlur={e => e.target.style.borderColor = T.surfaceBorder} />
...
function StepDot({ n, active, done }) {
  return (
    <div style={{
      ...
      background: done ? "#14B8A6" : active ? T.bg : T.surface,
      border: active || done ? "none" : `1px solid ${T.surfaceBorder}`,
      fontSize: 11, fontWeight: 700, color: done || active ? T.text : T.dim,
...
function LoadingState() {
  ...
  return (
    <div style={{ padding: "56px 0", textAlign: "center" }}>
      <div style={{ width: 56, height: 56, borderRadius: "50%", background: T.bg, display: "flex", alignItems: "center", justifyContent: "center", margin: "0 auto 24px" }}>
      ...
      <div style={{ fontSize: 13, color: T.dim, marginBottom: 32 }}>Claude is tailoring a presentation for your audience and goal</div>
      <div style={{ maxWidth: 300, margin: "0 auto" }}>
        {steps.map((s, i) => (
          <div key={i} style={{ display: "flex", gap: 10, alignItems: "center", marginBottom: 12, opacity: i <= activeStep ? 1 : 0.3, transition: "opacity 0.4s" }}>
            <div style={{ width: 18, height: 18, borderRadius: "50%", background: i < activeStep ? "#14B8A6" : i === activeStep ? T.bg : T.surface, border: i < activeStep ? "none" : `1px solid ${T.surfaceBorder}`, display: "flex", alignItems: "center", justifyContent: "center", flexShrink: 0, fontSize: 9, color: i < activeStep ? T.text : T.dim, fontWeight: 700 }}>{i < activeStep ? "✓" : i + 1}</div>
...
function SlideCard({ slide, isOpen, onToggle, accentColor }) {
  return (
    <div style={{ background: T.text, border: `1px solid ${isOpen ? accentColor + "55" : T.surfaceBorder}`, borderRadius: 12, marginBottom: 8, overflow: "hidden", transition: "border-color 0.2s" }}>
      <button onClick={onToggle} style={{ width: "100%", display: "flex", alignItems: "center", gap: 14, padding: "14px 18px", background: "none", border: "none", cursor: "pointer", textAlign: "left" }}>
        <div style={{ width: 28, height: 28, borderRadius: 6, background: isOpen ? accentColor : T.surface, border: `1px solid ${isOpen ? "transparent" : T.surfaceBorder}`, display: "flex", alignItems: "center", justifyContent: "center", flexShrink: 0, fontSize: 11, fontWeight: 700, color: isOpen ? T.text : T.dim, fontFamily: "'JetBrains Mono', monospace", transition: "all 0.2s" }}>
          {String(slide.slideNumber).padStart(2, "0")}
        </div>
        <div style={{ flex: 1 }}>
          <div style={{ fontSize: 13, fontWeight: 600, color: T.text }}>{slide.title}</div>
          <div style={{ fontSize: 11, color: T.dim, fontFamily: "'JetBrains Mono', monospace", marginTop: 2 }}>{slide.slideType}</div>
        </div>
        {slide.keyMessage && !isOpen && (
          <div style={{ fontSize: 11, color: T.dim, maxWidth: 280, textAlign: "right", lineHeight: 1.4, fontStyle: "italic" }}>"{slide.keyMessage}"</div>
        )}
      ...
      {isOpen && (
        <div style={{ borderTop: `1px solid ${T.surfaceBorder}`, padding: "18px 18px 20px" }}>
          <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 16, marginBottom: 16 }}>
            <div>
              <div style={{ fontSize: 10, fontFamily: "'JetBrains Mono', monospace", textTransform: "uppercase", letterSpacing: 2, color: T.dim, marginBottom: 8 }}>Speaker notes</div>
              <p style={{ fontSize: 13, color: T.text, lineHeight: 1.7, margin: 0 }}>{slide.speakerNotes}</p>
            </div>
            <div>
              <div style={{ fontSize: 10, fontFamily: "'JetBrains Mono', monospace", textTransform: "uppercase", letterSpacing: 2, color: T.dim, marginBottom: 8 }}>Key message</div>
              <p style={{ fontSize: 13, color: T.text, lineHeight: 1.7, margin: "0 0 14px", fontWeight: 500 }}>{slide.keyMessage}</p>
  ...
  return (
    <div style={{ animation: "fadeIn 0.4s ease" }}>
      ...
      <div style={{ marginBottom: 24 }}>
        <div style={{ fontSize: 10, fontFamily: "'JetBrains Mono', monospace", letterSpacing: 3, textTransform: "uppercase", color: T.dim, marginBottom: 10 }}>Deck Ready</div>
        <div style={{ display: "flex", alignItems: "flex-start", gap: 14, flexWrap: "wrap" }}>
          <div style={{ flex: 1 }}>
            <h2 style={{ fontFamily: "'DM Serif Display', serif", fontSize: 30, fontWeight: 400, color: T.text, margin: "0 0 6px", lineHeight: 1.1 }}>{deck.deckTitle}</h2>
            <div style={{ display: "flex", alignItems: "center", gap: 10, flexWrap: "wrap" }}>
              ...
              <span style={{ fontSize: 12, color: T.dim }}>{deck.estimatedSlides} slides</span>
            </div>
          </div>
        </div>
        <p style={{ fontSize: 13, color: T.dim, marginTop: 10, lineHeight: 1.7, maxWidth: 640 }}>{deck.deckTypeReason}</p>
      </div>

      {/* Narrative arc */}
      <div style={{ background: T.bg, borderRadius: 12, padding: "18px 22px", marginBottom: 14 }}>
      ...
      <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 12, marginBottom: 14 }}>
        <div style={{ background: T.text, border: `1px solid ${T.surfaceBorder}`, borderRadius: 12, padding: "16px 18px" }}>
          <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 10 }}>
            <div style={{ fontSize: 10, fontFamily: "'JetBrains Mono', monospace", textTransform: "uppercase", letterSpacing: 2, color: T.dim }}>Opening hook</div>
            <button onClick={onCopyHook} style={{ background: copiedHook ? accentColor : "transparent", color: copiedHook ? T.text : accentColor, border: `1px solid ${accentColor}55`, borderRadius: 6, padding: "4px 10px", fontSize: 10, fontWeight: 600, cursor: "pointer", fontFamily: "'DM Sans', sans-serif", transition: "all 0.2s" }}>{copiedHook ? "✓ Copied" : "Copy"}</button>
          </div>
          <p style={{ fontSize: 13, color: T.text, lineHeight: 1.7, margin: 0, fontStyle: "italic" }}>"{deck.openingHook}"</p>
        </div>
        <div style={{ background: T.text, border: `1px solid ${T.surfaceBorder}`, borderRadius: 12, padding: "16px 18px" }}>
          <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 10 }}>
            <div style={{ fontSize: 10, fontFamily: "'JetBrains Mono', monospace", textTransform: "uppercase", letterSpacing: 2, color: T.dim }}>Closing statement</div>
            <button onClick={onCopyClose} style={{ background: copiedClose ? accentColor : "transparent", color: copiedClose ? T.text : accentColor, border: `1px solid ${accentColor}55`, borderRadius: 6, padding: "4px 10px", fontSize: 10, fontWeight: 600, cursor: "pointer", fontFamily: "'DM Sans', sans-serif", transition: "all 0.2s" }}>{copiedClose ? "✓ Copied" : "Copy"}</button>
          </div>
          <p style={{ fontSize: 13, color: T.text, lineHeight: 1.7, margin: 0, fontStyle: "italic" }}>"{deck.closingStatement}"</p>
        </div>
      </div>

      {/* Slides */}
      <div style={{ marginBottom: 14 }}>
        <div style={{ fontSize: 10, fontFamily: "'JetBrains Mono', monospace", textTransform: "uppercase", letterSpacing: 2, color: T.dim, marginBottom: 14 }}>Slide-by-slide — click to expand</div>
        ...
        <div style={{ background: T.text, border: `1px solid ${T.surfaceBorder}`, borderRadius: 12, padding: "16px 18px" }}>
          ...
          {(deck.commonMistakesToAvoid || []).map((m, i) => (
            <div key={i} style={{ display: "flex", gap: 10, marginBottom: 8 }}>
              ...
              <span style={{ fontSize: 12, color: T.text, lineHeight: 1.55 }}>{m}</span>
            </div>
          ))}
        </div>
        <div style={{ background: T.text, border: `1px solid ${T.surfaceBorder}`, borderRadius: 12, padding: "16px 18px" }}>
          ...
          {(deck.followUpActions || []).map((a, i) => (
            <div key={i} style={{ display: "flex", gap: 10, marginBottom: 8 }}>
              ...
              <span style={{ fontSize: 12, color: T.text, lineHeight: 1.55 }}>{a}</span>
            </div>
          ))}
        </div>
      </div>

      <button onClick={onReset} style={{ background: T.text, border: `1px solid ${T.surfaceBorder}`, borderRadius: 8, padding: "10px 20px", fontSize: 13, fontWeight: 500, color: T.dim, cursor: "pointer", fontFamily: "'DM Sans', sans-serif" }}>← Build a new deck</button>
    </div>
  );
}
...
export default function ClientDeckBuilder() {
  ...
  return (
    <div style={{ minHeight: "100vh", background: T.surface, fontFamily: "'DM Sans', sans-serif" }}>
      ...
      {/* Top bar */}
      <div style={{ background: T.bg, borderBottom: `1px solid ${T.bgBorder}`, padding: "16px 40px", display: "flex", alignItems: "center", gap: 14 }}>
        ...
        <span style={{ fontSize: 14, fontWeight: 600, color: T.text }}>Client Deck Builder</span>
        <div style={{ marginLeft: "auto", fontSize: 11, color: T.muted, fontFamily: "'JetBrains Mono', monospace", opacity: 0.5 }}>Agentic Product Design Framework</div>
      </div>
          ...
          <>
            ...
            {step === 1 && (
              <div style={{ animation: "fadeIn 0.3s ease" }}>
                <h2 style={{ fontFamily: "'DM Serif Display', serif", fontSize: 28, fontWeight: 400, color: T.text, margin: "0 0 6px" }}>What does this deck need to accomplish?</h2>
                <p style={{ fontSize: 14, color: T.dim, margin: "0 0 28px", lineHeight: 1.6 }}>The goal and audience shape everything — the deck type, narrative, slide count, and tone.</p>

                <div style={{ marginBottom: 24 }}>
                  <div style={{ fontSize: 12, fontWeight: 600, color: T.text, marginBottom: 12 }}>What's the goal of this presentation?</div>
                  <div style={{ display: "flex", flexWrap: "wrap", gap: 8 }}>
                    {DECK_GOALS.map(g => (
                      <Chip key={g.id} selected={form.deckGoal === g.id} onClick={() => set("deckGoal", g.id)} color={accentColor}>
                        ...
                        <div>
                          <div>{g.label}</div>
                          <div style={{ fontSize: 10, color: T.dim, fontWeight: 400 }}>{g.desc}</div>
                        </div>
                      </Chip>
                    ))}
                  </div>
                ...
                <div style={{ marginBottom: 24 }}>
                  <div style={{ fontSize: 12, fontWeight: 600, color: T.text, marginBottom: 12 }}>Who is in the room?</div>
                  <div style={{ display: "flex", flexWrap: "wrap", gap: 8 }}>
                    {AUDIENCE_OPTIONS.map(a => (
                      <Chip key={a.id} selected={form.audience === a.id} onClick={() => set("audience", a.id)} color={accentColor}>
                        <div>
                          <div>{a.label}</div>
                          <div style={{ fontSize: 10, color: T.dim, fontWeight: 400 }}>{a.desc}</div>
                        </div>
                      </Chip>
                    ))}
                  </div>
                ...
                <button onClick={() => setStep(2)} disabled={!canNext1} style={{ background: canNext1 ? T.proto : T.card, color: canNext1 ? T.text : T.dim, border: "none", borderRadius: 10, padding: "13px 28px", fontSize: 14, fontWeight: 600, cursor: canNext1 ? "pointer" : "default", fontFamily: "'DM Sans', sans-serif", transition: "all 0.15s" }}>Continue →</button>
              </div>
            )}
            ...
            {step === 2 && (
              <div style={{ animation: "fadeIn 0.3s ease" }}>
                <h2 style={{ fontFamily: "'DM Serif Display', serif", fontSize: 28, fontWeight: 400, color: T.text, margin: "0 0 6px" }}>Where are you in the project?</h2>
                <p style={{ fontSize: 14, color: T.dim, margin: "0 0 28px", lineHeight: 1.6 }}>This tells Claude what work exists to reference and what the narrative arc should cover.</p>

                <div style={{ marginBottom: 24 }}>
                  <div style={{ fontSize: 12, fontWeight: 600, color: T.text, marginBottom: 6 }}>Phases completed <span style={{ fontWeight: 400, color: T.dim }}>(select all that apply)</span></div>
                  <div style={{ fontSize: 11, color: T.dim, marginBottom: 12 }}>These become the substance of the deck — what you've done, what you found, what you decided.</div>
                  <div style={{ display: "flex", flexWrap: "wrap", gap: 8 }}>
                    {PHASE_OPTIONS.map(p => (
                ...
                <div style={{ marginBottom: 24 }}>
                  <div style={{ fontSize: 12, fontWeight: 600, color: T.text, marginBottom: 12 }}>Presentation tone</div>
                  <div style={{ display: "flex", flexWrap: "wrap", gap: 8 }}>
                    {TONE_OPTIONS.map(t => (
                      <Chip key={t.id} selected={form.tone === t.id} onClick={() => set("tone", t.id)} color={accentColor}>
                        <div>
                          <div>{t.label}</div>
                          <div style={{ fontSize: 10, color: T.dim, fontWeight: 400 }}>{t.desc}</div>
                        </div>
                      </Chip>
                    ))}
                  </div>
                ...
                <div style={{ marginBottom: 24 }}>
                  <div style={{ fontSize: 12, fontWeight: 600, color: T.text, marginBottom: 12 }}>Deck length</div>
                ...
                <div style={{ display: "flex", gap: 10 }}>
                  ...
                  <button onClick={() => setStep(3)} disabled={!canNext2} style={{ background: canNext2 ? T.proto : T.card, color: canNext2 ? T.text : T.dim, border: "none", borderRadius: 10, padding: "13px 28px", fontSize: 14, fontWeight: 600, cursor: canNext2 ? "pointer" : "default", fontFamily: "'DM Sans', sans-serif", transition: "all 0.15s" }}>Continue →</button>
                </div>
              </div>
            )}
            ...
            {step === 3 && (
              <div style={{ animation: "fadeIn 0.3s ease" }}>
                <h2 style={{ fontFamily: "'DM Serif Display', serif", fontSize: 28, fontWeight: 400, color: T.text, margin: "0 0 6px" }}>Content & context</h2>
                <p style={{ fontSize: 14, color: T.dim, margin: "0 0 28px", lineHeight: 1.6 }}>The more specific you are, the more tailored the speaker notes and slide content will be.</p>
                ...
                <div style={{ display: "flex", gap: 10, marginTop: 8 }}>
                  ...
                  <button onClick={generate} disabled={!canSubmit} style={{ background: canSubmit ? accentColor : T.surfaceBorder, color: canSubmit ? T.bg : T.dim, border: "none", borderRadius: 10, padding: "13px 28px", fontSize: 14, fontWeight: 700, cursor: canSubmit ? "pointer" : "default", fontFamily: "'DM Sans', sans-serif", transition: "all 0.15s" }}>
                    Build Deck ◆
                  </button>
                </div>
              </div>
